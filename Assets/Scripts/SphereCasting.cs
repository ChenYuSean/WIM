 ...
 * To active this script, please use the GameManager in Unity and change the mode to SphereCasting.<br/>
 * SphereCasting is the main script controll over bubble mechanism and two step selection.<br/>
 * It gets the current step in GameManager and controll the step flow in Update(),<br/>
 * In default step, bubble mechanism is used, and in two step method, the Sphere Casting method is used.<br/>
 * </summary>
 */
public class SphereCasting : MonoBehaviour
{
    ...
    private SteamVR_Action_Boolean m_GripPress;
    ...
    private GameObject CastingSphere;
    ...
    private float DeltaSphereRadius = 0f;
    private float DeltaSphereDis = 0f;
    ...
    void Awake() {
        CastingSphere = GameObject.Find("MainSphere");
        ...
        SphereSize = 5f;
        CastingSphere.transform.localScale = new Vector3(SphereSize, SphereSize, SphereSize);
    }
    ...
    void Start()
    {
        ...
        m_GripPress = SteamVR_Input.GetAction<SteamVR_Action_Boolean>("PressGrabGrip");
        ...
        switch (NOWSTEP)
        {
            case STEP.dflt:
                ...
                if (Physics.Raycast(RayOriginR, RayDirectionR, out rhit, RayLengthR, layerMask))
                {
                    ...
                    // default -> Step1
                    if (vec2TouchpadAxis.y != 0)
                    {
                        SphereSize = 4 * Mathf.Max(rhit.collider.bounds.size.x, rhit.collider.bounds.size.y, rhit.collider.bounds.size.z);
                        CastingSphere.transform.localScale = new Vector3(SphereSize, SphereSize, SphereSize);
                        CastingSphere.transform.position = RayDestinationR;
                        SphereDis = Vector3.Distance(CastingSphere.transform.position, RayOriginR);
                        CastingSphere.SetActive(true);
            ...
            case STEP.One:
                ...
                AdjustSphereDisAndSize();

                RayEndR.SetActive(false);
                BubbleDiskR.SetActive(false);

                RayDestinationR = RayOriginR + RayDirectionR * SphereDis;
                CastingSphere.transform.position = RayDestinationR;
                ...
                FindBySphere(CastingSphere.transform.position, CastingSphere.transform.localScale.x / 2, layerMask);
                ...
                if (isTriggerPressR)
                {
                    ...
                    oriSphereSize = CastingSphere.transform.localScale.x;
                    ...
                    CreateReplicas(CastingSphere.transform.position, CastingSphere.transform.localScale.x / 2, layerMask);
            ...
            case STEP.Two:
                ...
                if (isGripPressR || isGripPressL)
                {
                    ...
                    CastingSphere.transform.localScale = new Vector3(oriSphereSize, oriSphereSize, oriSphereSize);
                    GameManager.Instance.SetCurStep(STEP.One);
                    return;
                }
                ...
                // Enlarge
                if (isMenuPressR || (vec2TouchpadAxis.x >= 0.5f && isTouchpadRightPress))
                {
                    if (!Enlarge)
                    {
                ...
                // Leave Enlarge
                if (isMenuPressL || (vec2TouchpadAxis.x <= 0.5f && isTouchpadLeftPress))
                {
                    if (Enlarge)
                    {
                        Enlarge = false;
                ...
                if (Enlarge) 
                {
                    recorder.time_DistanceEnlarging += Time.deltaTime;
                }
                break;
        }
    }
    private void AdjustSphereDisAndSize()
    {
        if (vec2TouchpadAxis.y > 0.5 && isTouchpadUpPress)
        {
            DeltaSphereDis = 0.5f * CastingSphere.transform.localScale.x;
            SphereDis += DeltaSphereDis;
        }
        ...
        {
            DeltaSphereRadius = 0.1f * CastingSphere.transform.localScale.x;
            CastingSphere.transform.localScale += new Vector3(DeltaSphereRadius, DeltaSphereRadius, DeltaSphereRadius);
        }
    ...
    private void SetHighlight(GameObject obj, string type, bool OnOff)
    {
        ...
        {
            Debug.Log("[Error]Trying to set highlight on null object");
        }
    }
    ...
    private void CreateReplicas(Vector3 center, float radius, LayerMask layerMask)
    {
        ...
        Collider[] coveredTargets = Physics.OverlapSphere(center, radius, layerMask);
        int i = 0, j = 0;
    ...
    private void FindBySphere(Vector3 center, float radius, LayerMask layerMask)
    {
        float offset = (Vector3.Angle(Vector3.up, Cam.transform.forward) - 90)/ 90.0f;
        SpherePos = Cam.transform.position + CamFrontProjXZ *11/20 + CamDownParaY / 5 + CamDownParaY * offset - CamDownParaY / 6.5f;

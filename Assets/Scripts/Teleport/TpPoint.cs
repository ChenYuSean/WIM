...
//-------------------------------------------------------------------------
public class TpPoint : TeleportMarkerBase
{
	...
	{
		if(teleport == null)
			teleport = FindObjectOfType<Teleportation>();
	}
	...
	void Update()
	{
		...
		{
			lookAtPosition.x = Cam.transform.position.x;
			lookAtPosition.y = lookAtJointTransform.position.y;
			lookAtPosition.z = Cam.transform.position.z;
	...
	public override void Highlight( bool highlight )
	{
		if ( !locked )
		{
			if ( highlight )
			{
				SetMeshMaterials( teleport.pointHighlightedMaterial, titleHighlightedColor );
			}
			else
			{
				SetMeshMaterials( teleport.pointVisibleMaterial, titleVisibleColor );
			}
		}
	...
	public override void UpdateVisuals()
	{
		...
		if ( locked )
		{
			SetMeshMaterials( teleport.pointLockedMaterial, titleLockedColor );
	...
	public void UpdateVisualsInEditor()
	{
		...
		if ( locked )
		{
			...
			markerMesh.sharedMaterial = teleport.pointLockedMaterial;
			lockedIcon.sharedMaterial = teleport.pointLockedMaterial;
		...
		else
		{
			...
			markerMesh.sharedMaterial = teleport.pointVisibleMaterial;
			switchSceneIcon.sharedMaterial = teleport.pointVisibleMaterial;
			moveLocationIcon.sharedMaterial = teleport.pointVisibleMaterial;
